...
blue = c(0.25, 0.25, 1))
{
    rr <- table.ramp(n, mid = red[[1]], sill = red[[2]], base = red[[3]])
    gr <- table.ramp(n, mid = green[[1]], sill = green[[2]], base = green[[3]])
    br <- table.ramp(n, mid = blue[[1]], sill = blue[[2]], base = blue[[3]])
    rgb(rr, gr, br)
}
