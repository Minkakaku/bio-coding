...
exp.list.file <- "list/sce.CD8.fullPath.list"
sce <- readRDS("OUT.int.CD8/int.CD8.sce.merged.rds")
seu <- readRDS("OUT.int.CD8/int.CD8.seu.merged.rds")
meta.tb <- readRDS("OUT.int.CD8/int.CD8.meta.tb.rds")

out.prefix <- "OUT.int.CD8/int.CD8"
...

{
    ...
    sce$meta.cluster[sce$ClusterID %in% c("C16","C04")] <- "CD8.c01.Tn.MAL"
    sce$meta.cluster[sce$ClusterID %in% c("C15","C10","C33","C06","C28")] <- "CD8.c02.Tm.IL7R"
    sce$meta.cluster[sce$ClusterID %in% c("C03")] <- "CD8.c03.Tm.RPS12"
    sce$meta.cluster[sce$ClusterID %in% c("C17","C18")] <- "CD8.c04.Tm.CD52"
    sce$meta.cluster[sce$ClusterID %in% c("C01","C24","C31","C13","C20","C12")] <- "CD8.c05.Tem.CXCR5"
    sce$meta.cluster[sce$ClusterID %in% c("C14","C23")] <- "CD8.c06.Tem.GZMK"
    sce$meta.cluster[sce$ClusterID %in% c("C05","C08","C27")] <- "CD8.c07.Temra.CX3CR1"
    sce$meta.cluster[sce$ClusterID %in% c("C30")] <- "CD8.c08.Tk.TYROBP"
    sce$meta.cluster[sce$ClusterID %in% c("C25")] <- "CD8.c09.Tk.KIR2DL4"
    sce$meta.cluster[sce$ClusterID %in% c("C07","C21","C22","C02")] <- "CD8.c10.Trm.ZNF683"
    sce$meta.cluster[sce$ClusterID %in% c("C19","C34")] <- "CD8.c11.Tex.PDCD1"
    sce$meta.cluster[sce$ClusterID %in% c("C00","C35")] <- "CD8.c12.Tex.CXCL13"
    sce$meta.cluster[sce$ClusterID %in% c("C26")] <- "CD8.c13.Tex.myl12a"
    sce$meta.cluster[sce$ClusterID %in% c("C32")] <- "CD8.c14.Tex.TCF7"
    sce$meta.cluster[sce$ClusterID %in% c("C11")] <- "CD8.c15.ISG.IFIT1"
    sce$meta.cluster[sce$ClusterID %in% c("C09")] <- "CD8.c16.MAIT.SLC4A10"
    sce$meta.cluster[sce$ClusterID %in% c("C29")] <- "CD8.c17.Tm.NME1"
