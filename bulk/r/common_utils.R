...
read_table_by_extension <- function(file_path, header = TRUE) {
  ...
  if (ext %in% c("xlsx", "xls")) {
    if (!requireNamespace("readxl", quietly = TRUE)) {
      stop("Please install readxl to read Excel files: ", file_path)
    }
    return(as.data.frame(readxl::read_excel(file_path)))
  }
...
read_manifest_table <- function(file_path, required_columns) {
  manifest <- read_table_by_extension(file_path, header = TRUE)
  missing_cols <- setdiff(required_columns, colnames(manifest))
