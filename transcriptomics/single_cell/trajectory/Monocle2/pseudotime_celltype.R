...
library(cowplot)
# rm(list = ls())
# setwd("/home/wzb/Pseudotime/Neutrophil/")
...
if(is.null(args$RandomCells)){args$RandomCells= NULL}
...
Psepre= function(seuset,celllist,cluster,RandomCells,cellsPercent,minCellsPerCluster){
  ...
  if(!is.null(RandomCells)){
    newcelllist = randomSample(seuset@active.ident,cellsPercent = cellsPercent,minCellsPerCluster = minCellsPerCluster)
...
mainfun = function(seuset,geneFile,lambda,RootState,Reverse,celllist,cluster,RandomCells,
  ...
  seuCDS = ImportSeurat4CDS(seuset) %>% setOrderingFilter(genelist) %>%
    estimateSizeFactors() %>% 
    reduceDimension(reduction_method = "DDRTree",lambda=lambda) %>% orderCells()
...
# Neurds = readRDS("/home/wzb/Pseudotime/Neutrophil/Neutrophil.rds")
# markerlist = FindAllMarkers(Neurds,only.pos = T)
# write.table(markerlist,"/home/wzb/Pseudotime/Neutrophil/Recluster/Mannul.marker.txt",sep = "\t",quote = F,row.names = F)
# rm(list = ls())
# cds_subset = readRDS("/home/wzb/Pseudotime/Neutrophil/l8Result/Pseudotime.monocle.rds")
# library(dplyr)
