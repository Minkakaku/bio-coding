...
```{r}
dir <- "/home/hongfan/XMU2021/JOBs/ST_seq/paper_practice"
setwd(dir)
```
...
cell_names <- colnames(sc)
genes <- sc[-(1:1), 1] # Extract genes
sc <- t(apply(sc[-(1:1), -(1:1)], 1, as.numeric)) # Remove genes and cluster annotations from expression matrix
rownames(sc1) <- make.names(genes, unique = TRUE) # Set rownames of expression matrix to the list of genes
colnames(sc1) <- make.names(cell_names, unique = TRUE)
...
for (i in colnames(st)) {
  tissue_coord[i, "X"] <- (strsplit(sub("X", "", i), "x") %>% unlist())[1] %>% as.integer()
  tissue_coord[i, "Y"] <- (strsplit(sub("X", "", i), "x") %>% unlist())[2] %>% as.integer()
}
```
...
tissue_coord$clusters <- Idents(st) %>%
  unlist() %>%
  as.character()
...
#   cell.type.markers, # Common genes between these two gene sets
