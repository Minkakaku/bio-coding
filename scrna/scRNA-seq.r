...
v <- venn.diagram(
    list(
        UMI = rownames(colData(sce)[sce$nUMI.outlier.low, ]),
...
seuratObj <- FindNeighbors(seuratObj,
    dims = 1:dims.use, reduction = paste0("harmony_theta", theta.use),
...
for (res in c(0.25, 0.5, 0.75, 1)) {
    plot[[as.character(res)]] <- DimPlot(seur,
        pt.size = 1, label = T, repel = T, group.by = paste0("RNA_snn_harmony_theta", theta.use, ".PC", dims.use, "_res.", res),
